...
const Wrapper = styled(View) <{ focused: boolean }>`
  ...
  max-width: 720px;
  position: relative;
  bottom: ${({ focused }) => (focused ? true && '100px' : '0')};
  ...
  render() {
    ...
    return (
      <Wrapper focused={!!(inputFocused && address)}>
        ...
        />
        {/* <Script
          url={`https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=-33.8670522,151.1957362&radius=500&types=food&name=harbour&key=${GOOGLE_MAPS_KEY}`}
          onLoad={this.onLoadScript}
        /> */}
      </Wrapper>
    )
